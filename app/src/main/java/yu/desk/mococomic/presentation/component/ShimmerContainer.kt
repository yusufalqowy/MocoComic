...
class ShimmerContainer @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(context, attrs, defStyleAttr) {
    ...
    init {
        ...
        try {
            ...
            shapeAppearance = typedArray.getResourceId(R.styleable.ShimmerContainer_shapeAppearance, com.google.android.material.R.style.ShapeAppearance_Material3_Corner_Medium)
            shapeAppearanceOverlay = typedArray.getResourceId(R.styleable.ShimmerContainer_shapeAppearanceOverlay, com.google.android.material.R.style.ShapeAppearance_Material3_Corner_Medium)
            shimmerWidth = typedArray.getDimensionPixelSize(R.styleable.ShimmerContainer_shimmerWidth, ViewGroup.LayoutParams.MATCH_PARENT)
            shimmerHeight = typedArray.getDimensionPixelSize(R.styleable.ShimmerContainer_shimmerHeight, ViewGroup.LayoutParams.MATCH_PARENT)
    ...
    private fun initView() {
        ...
        binding.cvShimmer.shapeAppearanceModel = ShapeAppearanceModel.builder(context, shapeAppearance, shapeAppearanceOverlay).build()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val totalWidth = (if (shimmerWidth == ViewGroup.LayoutParams.MATCH_PARENT) measuredWidth else shimmerWidth).times(shimmerWidthPercent).roundToInt()
        val totalHeight = (if (shimmerHeight == ViewGroup.LayoutParams.MATCH_PARENT) measuredHeight else shimmerHeight).times(shimmerHeightPercent).roundToInt()
        binding.shimmerLayout.measure(MeasureSpec.makeMeasureSpec(totalWidth, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(totalHeight, MeasureSpec.EXACTLY))
    }
