...
sealed class ApiState<T> {
    class Success<T>(val data: T, val fromDb: Boolean = false) : ApiState<T>()

    class Failure<T>(val code: Int, val message: String) : ApiState<T>()

    class Error<T>(val message: String) : ApiState<T>()
}
